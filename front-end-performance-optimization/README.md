...
js资源是否要通过动态引入方式，在任何我们需要js资源的时候才去异步加载。单页面应用，当路由到单页面某个页面时，才去加载那个页面所对应的js文件。
...
css阻塞<br>
css head中阻塞页面的渲染：避免页面的闪动，css在head中通过link的方式引入，会阻塞页面的渲染。阻塞页面的渲染的意思是这个页面要呈现出相应的效果就需要link所对应的css加载完之后它才会去进行渲染。css加载完并分析完了，它整个渲染是带样式的。<br>
css阻塞js的执行<br><br>
css不阻塞外部脚本的加载<br>
